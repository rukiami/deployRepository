...
def get_event_data(request, date):
    ...
    # ... get_event_data 関数の定義 ...  

# ここに get_events 関数を追加
def get_events(request):
    # イベントのリストを取得して返すロジック
    events = Event.objects.all()
    data = [{'title': event.title, 'date': event.date.isoformat()} for event in events]
    return JsonResponse(data, safe=False)    
...
def add_event(request):
    try:
        ...
        # 日付フォーマットのエラーが発生した場合
        logger.error(f"Date Value Error: {date_error}")
