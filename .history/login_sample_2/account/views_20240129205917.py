    ...
    try:
        ...
        return JsonResponse({
            ...
            'event': {
                'id': event.id,
                'title': event.title, 
                'date': event.date.isoformat()  # 日付オブジェクトをISOフォーマットの文字列に変換
            }
        })
    
def get_events(request):
    # ここにイベントデータを取得してJSONで返すコードを実装
    events = Event.objects.all().values('title', 'start', 'end')  # 必要なフィールドを選択
